...
namespace CornerRecordsLMS.Controllers
{
    [ApiController]
    public class DesignAutomationController : ControllerBase
    {
        ...
        private dynamic EngineAttributes(string engine)
        {
            ...
            if (engine.Contains("AutoCAD")) return new { commandLine = "$(engine.path)\\accoreconsole.exe /i \"$(args[inputFile].path)\" /al \"$(appbundles[{0}].path)\" /s $(settings[script].path)", extension = "dwg", script = "OCPWCR\n" };
        ...
        public async Task<IActionResult> CreateActivity([FromBody] JObject activitySpecs)
        {
                    ...
                    Parameters = new Dictionary<string, Parameter>()
            {
                ...
                { "result", new Parameter() { Description = "output file", LocalName = "CornerRecordForms.json", Ondemand = false, Required = true, Verb = Verb.Put, Zip = false } }
            },
                ...
                Headers = new Dictionary<string, string>()
        {
            ...
            // };
            // 3. output file
            string outputFileNameOSS = string.Format("{0}_output_{1}", DateTime.Now.ToString("yyyyMMddhhmmss"), "CornerRecordForms.json"); // avoid overriding
